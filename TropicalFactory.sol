...
contract TropicalFactory is ITropicalFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'TropicalDEX: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TropicalDEX: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'TropicalDEX: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'TropicalDEX: FORBIDDEN');
        feeTo = _feeTo;
    }
